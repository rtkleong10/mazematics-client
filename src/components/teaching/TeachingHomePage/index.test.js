...
it('should load topics', () => {
    const { getByTestId } = render(renderWithReduxRouter(<TeachingHomePage />)); 
    expect(getByTestId('loader')).toBeTruthy();
});

it('should load and display the topics', async () => {
    const { getByText } = render(renderWithReduxRouter(<TeachingHomePage />));
...
it('should take a snapshot', async () => {
    const { asFragment, getByText } = render(renderWithReduxRouter(<TeachingHomePage />));
